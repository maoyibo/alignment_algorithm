...
from alignment_algorithm.utils import base_match,  print_matrix, align_report_gotoh
...
def compute_scoring_matrix(seq1: str,
                           ...
                           mismatch: int = -1,
                           gap_open: int = -3,
                           gap_extend: int = -1) -> Tuple[List[List[int]], List[List[int]], List[List[int]]]:
        ...
        for j in range(len(seq2)+1):
            if i == 0:  # i 不变，j 增加 表示从水平的加减
                ...
                scoring_matrix[i][j] = max(diagonal_value,
                                           ...
                                           vertical_matrix[i][j])
    return scoring_matrix, horizontal_matrix, vertical_matrix
...
def global_traceback(scoring_matrix:  List[List[int]],
                     horizontal_matrix:  List[List[int]],
                     vertical_matrix:  List[List[int]],
                     seq1: str,
                     seq2: str,
...
def gotoh(seq1: str,
    ...
    """Trace function
    """
    scoring_matrix, horizontal_matrix, vertical_matrix = compute_scoring_matrix(seq1=seq1,
                                                                                seq2=seq2,
                                                                                match=match,
                                                                                mismatch=mismatch,
                                                                                gap_open=gap_open,
